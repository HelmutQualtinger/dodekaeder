...
- `nudeldrucker.html` — same engine as `gemini.html` (literally forked from it), themed around 12 Austrian public figures spanning entertainment/media/politics/literature: Richard Lugner, DJ Ötzi, Heinz Fischer, Leonore Gewessler, Andreas Babler, Armin Wolf, Conchita Wurst, Alexander Van der Bellen, Hermes Phettberg, Karl Merkatz, Erika Pluhar, Elfriede Jelinek. Its own manifest (`manifest-nudeldrucker.json`) and social-preview image (`images/social-preview-nudeldrucker.jpg`), sharing `sw.js` and `icons/` with `gemini.html`. See "Content standard" below before adding/replacing anyone on this page.
